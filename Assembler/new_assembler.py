...
class Assembler:
    ...
    def encode_generic_jump(self, opcode_str, parts):
        if len(parts) != 2:
            raise ValueError(f"{parts[0]} requires 1 operand (label or immediate)")
        imm = self.parse_immediate(parts[1])
        word1 = self.create_instruction_word(opcode_str, {(31, 27): opcode_str})
        return [word1, imm]
    ...
    def parse_line(self, line):
        ...
        encoders = {
            ...
            "LDM": self.encode_ldm,
            "JZ": lambda p: self.encode_generic_jump("01011", p),
            "JN": lambda p: self.encode_generic_jump("01100", p),
            "JC": lambda p: self.encode_generic_jump("01101", p),
            "JMP": lambda p: self.encode_generic_jump("01110", p),
            "CALL": lambda p: self.encode_generic_jump("01111", p),
            "RET": self.encode_ret,
            "INT": self.encode_int,
            "RTI": self.encode_rti,
            "LDD": self.encode_ldd,
    ...
    def assemble(self, input_file, output_file=None):
        ...
        # PASS 1: Identify Labels and Addresses
        current_address = self.CODE_SEGMENT_START
        instructions_only = []
        
        # Two-word instruction list for address calculation
        two_word_ops = ["IADD", "LDM", "JZ", "JN", "JC", "JMP", "CALL", "LDD", "STD"]

        for line in raw_lines:
            line = line.split("#")[0].strip()
            if not line: continue
            
            # Check for label (e.g., "myLoop:")
            if ":" in line:
                label_part, rest = line.split(":", 1)
                self.labels[label_part.strip().upper()] = current_address
                line = rest.strip()
                if not line: continue
        ...
        if output_file:
            ...
            with open("../" + output_file, "w") as f:
                f.write(f"0:{self.CODE_SEGMENT_START:08x}".upper() + "\n")
                f.write(f"1:{Interrupt1:08x}".upper() + "\n")
                f.write(f"2:{Interrupt2:08x}".upper() + "\n")
                f.write(f"3:{Interrupt3:08x}".upper() + "\n")
                for word in machine_code:
                    f.write(f"{lineCount+self.CODE_SEGMENT_START}:{int(word,2):08x}".upper() + "\n")
