...
class Assembler:
    ...
    def parse_immediate(self, imm_str):
        ...
        if imm_str.startswith('0x') or imm_str.startswith('0X') or imm_str.startswith('-0x') or imm_str.startswith('-0X'):
            value = int(imm_str, 16)
        elif imm_str.startswith('0b') or imm_str.startswith('0B') or imm_str.startswith('-0b') or imm_str.startswith('-0B'):
            value = int(imm_str, 2)
        else:
            value = int(imm_str)
    ...
    def encode_setc(self, parts):
        # SETC
        return [self.create_instruction_word('00010', {
            (31, 27): '00010',
            (2, 0): '100'
        })]
    ...
    def encode_movs(self, parts):
        ...
        rdst = self.parse_register(parts[1].rstrip(','))
        rsrc = self.parse_register(parts[2])
        return [self.create_instruction_word('01000', {
            (31, 27): '01000',
            (26, 24): rdst,
            (23, 21): rsrc,
            (2, 0): '011'
    ...
    def encode_swap(self, parts):
        ...
        rdst = self.parse_register(parts[1].rstrip(','))
        rsrc = self.parse_register(parts[2])
        return [self.create_instruction_word('11111', {
            (31, 27): '11111',
            (26, 24): rdst,
            (23, 21): rsrc,
            (2, 0): '011'
    ...
    def encode_add(self, parts):
        # ADD
        if len(parts) != 4:
            raise ValueError("ADD instruction requires 3 operands: ADD Rdst, Rsrc1, Rsrc2")
        rdst = self.parse_register(parts[1].rstrip(','))
    ...
    def encode_sub(self, parts):
        # SUB
        if len(parts) != 4:
            raise ValueError("SUB instruction requires 3 operands: SUB Rdst, Rsrc1, Rsrc2")
        rdst = self.parse_register(parts[1].rstrip(','))
        rsrc1 = self.parse_register(parts[2].rstrip(','))
        rsrc2 = self.parse_register(parts[3])
        return [self.create_instruction_word('00010', {
    ...
    def encode_and(self, parts):
        # AND
        if len(parts) != 4:
            raise ValueError("AND instruction requires 3 operands: AND Rdst, Rsrc1, Rsrc2")
        rdst = self.parse_register(parts[1].rstrip(','))
        rsrc1 = self.parse_register(parts[2].rstrip(','))
        rsrc2 = self.parse_register(parts[3])
        return [self.create_instruction_word('00010', {
    ...
    def encode_out(self, parts):
        ...
        return [self.create_instruction_word('00011', {
            (31, 27): '00011',
            (23, 21): rsrc1
        })]
    ...
    def encode_in(self, parts):
        ...
        rdst = self.parse_register(parts[1])
        return [self.create_instruction_word('00100', {
            (31, 27): '00100',
            (26, 24): rdst
        })]
    ...
    def encode_push(self, parts):
        ...
        return [self.create_instruction_word('00110', {
            (31, 27): '00110',
            (20, 18): rsrc2
        })]
    ...
    def encode_pop(self, parts):
        ...
        rdst = self.parse_register(parts[1])
        return [self.create_instruction_word('00111', {
            (31, 27): '00111',
            (26, 24): rdst
        })]
    ...
    def encode_iadd(self, parts):
        # IADD
        if len(parts) != 4:
            raise ValueError("IADD instruction requires 3 operands: IADD Rdst, Rsrc, Imm")
        rdst = self.parse_register(parts[1].rstrip(','))
        rsrc = self.parse_register(parts[2].rstrip(','))
        imm = self.parse_immediate(parts[3])
        
        word1 = self.create_instruction_word('01001', {
            (31, 27): '01001',
            (26, 24): rdst,
            (23, 21): rsrc
        })
    ...
    def encode_ldm(self, parts):
        ...
        word1 = self.create_instruction_word('01010', {
            (31, 27): '01010',
            (26, 24): rdst
        })
        return [word1, imm]
    ...
    def encode_jz(self, parts):
        ...
        word1 = self.create_instruction_word('01011', {
            (31, 27): '01011'
        })
        return [word1, imm]
    ...
    def encode_jn(self, parts):
        ...
        word1 = self.create_instruction_word('01100', {
            (31, 27): '01100'
        })
        return [word1, imm]
    ...
    def encode_jc(self, parts):
        ...
        word1 = self.create_instruction_word('01101', {
            (31, 27): '01101'
        })
        return [word1, imm]
    ...
    def encode_jmp(self, parts):
        ...
        word1 = self.create_instruction_word('01110', {
            (31, 27): '01110'
        })
        return [word1, imm]
    ...
    def encode_call(self, parts):
        ...
        word1 = self.create_instruction_word('01111', {
            (31, 27): '01111'
        })
        return [word1, imm]
    ...
    def encode_ret(self, parts):
        # RET
        return [self.create_instruction_word('10000', {
            (31, 27): '10000'
        })]
    ...
    def encode_int(self, parts):
        ...
        index_bit = index
        return [self.create_instruction_word('10001', {
            (31, 27): '10001',
            (0, 0): index_bit
        })]
    ...
    def encode_rti(self, parts):
        # RTI
        return [self.create_instruction_word('10010', {
            (31, 27): '10010'
        })]
    ...
    def encode_ldd(self, parts):
        # LDD
        if len(parts) != 3:
            raise ValueError("LDD instruction requires 2 operands: LDD Rdst, offset(Rsrc)")
        
        # Parse offset(Rsrc) format
        offset_rsrc = parts[2]
        match = re.match(r'(-?\d+|0x[0-9A-Fa-f]+|0b[01]+)\(R(\d+)\)', offset_rsrc)
        if not match:
            raise ValueError(f"Invalid LDD format: {offset_rsrc}. Expected: offset(Rsrc)")
        
        offset = self.parse_immediate(match.group(1))
        rsrc = self.parse_register(f"R{match.group(2)}")
        rdst = self.parse_register(parts[1].rstrip(','))
        
        word1 = self.create_instruction_word('10011', {
            (31, 27): '10011',
            (26, 24): rdst,
            (23, 21): rsrc
        })
    ...
    def encode_std(self, parts):
        # STD
        if len(parts) != 3:
            raise ValueError("STD instruction requires 2 operands: STD Rsrc2, offset(Rsrc1)")
        
        # Parse offset(Rsrc) format
        offset_rsrc = parts[2]
        match = re.match(r'(-?\d+|0x[0-9A-Fa-f]+|0b[01]+)\(R(\d+)\)', offset_rsrc)
        if not match:
            raise ValueError(f"Invalid STD format: {offset_rsrc}. Expected: offset(Rsrc1)")
        
        offset = self.parse_immediate(match.group(1))
        rsrc1 = self.parse_register(f"R{match.group(2)}")
        rsrc2 = self.parse_register(parts[1].rstrip(','))
        
        word1 = self.create_instruction_word('10100', {
            (31, 27): '10100',
            (23, 21): rsrc1,
            (20, 18): rsrc2
        })
